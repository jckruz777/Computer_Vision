...
class AnomalyDetector:
    ...
    def evaluate(self, reconstruction_error, orig, rec):
        if reconstruction_error < self._anomaly_treshold:
            ...
            sub = cv2.subtract(cv2.resize(orig, (200, 200)), cv2.resize(rec, (200, 200)))
            markers = self._segment(sub)
            anomalies = cv2.resize(orig, (200, 200))

            cv2.imshow('Original', cv2.resize(orig, (200, 200)))
            cv2.imshow('Reconstruction', np.array(cv2.resize(rec, (200, 200)), dtype = np.uint8 ))
            anomalies[markers == -1] = [255,0,0]
            cv2.imshow('Diff', cv2.resize(anomalies, (200, 200)))
